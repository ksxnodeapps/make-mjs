import { fromFileWithoutChecking, fromFile } from '@make-mjs/url'
...
describe('fromFile', () => {
  ...
  describe('when forceMjs is false and .mjs files exist', () => {
    const get = (modulePath: string) => fromFile({ modulePath, forceMjs: false, moduleContainer: [''] })
  ...
  describe('when forceMjs is true', () => {
    const get = (modulePath: string) => fromFile({ modulePath, forceMjs: true, moduleContainer: [''] })
